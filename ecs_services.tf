resource "aws_ecs_service" "devnology_api_service" {
  ...
  load_balancer {
    target_group_arn = aws_lb_target_group.devnology_api_tg.arn
    container_name   = "devnology-api"
    container_port   = 3001
  }

  depends_on = [
    aws_lb_listener.devnology_api_listener
  ]
}
...
resource "aws_lb_target_group" "devnology_api_tg" {
  name        = "devnology-api-tg"
  port        = 3001
  protocol    = "HTTP"
  vpc_id      = data.aws_vpc.vpc.id
  target_type = "ip"
...
resource "aws_lb_listener" "devnology_api_listener" {
  load_balancer_arn = aws_lb.devnology_alb.arn
  port              = "3001"
  protocol          = "HTTP"
